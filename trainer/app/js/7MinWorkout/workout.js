...
function WorkoutRunnerController($interval, $location) {
    ...
    var startWorkout = function () {
        ctrl.workoutPlan = createWorkout();
        ctrl.workoutTimeRemaining = ctrl.workoutPlan.totalWorkoutDuration();
        restExercise = {
            ...
            duration: ctrl.workoutPlan.restBetweenExercise
        };
        $interval(function () {
            ctrl.workoutTimeRemaining = ctrl.workoutTimeRemaining - 1;
        }, 1000, ctrl.workoutTimeRemaining);

        startExercise(ctrl.workoutPlan.exercises.shift());
    };

    var startExercise = function (exercisePlan) {
        ctrl.currentExercise = exercisePlan;
        ctrl.currentExerciseDuration = 0;
        $interval(function () {
            ++ctrl.currentExerciseDuration;
        }, 1000, ctrl.currentExercise.duration)
            .then(function () {
    ...
    var getNextExercise = function (currentExercisePlan) {
        ...
        if (currentExercisePlan === restExercise) {
            nextExercise = ctrl.workoutPlan.exercises.shift();
        }
        else {
            if (ctrl.workoutPlan.exercises.length != 0) {
    ...
    //ctrl.$watch('currentExerciseDuration', function (nVal) {
    //    if (nVal == ctrl.currentExercise.duration) {
    //        var next = getNextExercise(ctrl.currentExercise);
    //        if (next) {
    //            startExercise(next);
    //        } else {
